...
const App = () => {
  const [numbers, setNumbers] = useState(Array.from({ length: 40 }, (_, i) => i + 1));
  ...
  const spinWheel = () => {
    ...
    const spinAmount = 360 * (5 + Math.random() * 5) + (chosen / 40) * 360;
  ...
  // Floating sparkles / confetti data
    ...
    <div className="relative flex flex-col min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-800 text-white overflow-hidden">
      ...
      <main className="flex flex-col items-center justify-center flex-grow p-4 relative">
        ...
        <div className="relative w-[250px] h-[250px] sm:w-[350px] sm:h-[350px] md:w-[450px] md:h-[450px] lg:w-[550px] lg:h-[550px] flex items-center justify-center">
          {/* Outer Glow + Golden border */}
          <div className="absolute w-[115%] h-[115%] rounded-full blur-2xl opacity-50 animate-pulse"
               style={{ background: "radial-gradient(circle, #FFD700 0%, #FFC700 70%, #FFB700 100%)" }} />

          {/* Wheel with thin inner golden border */}
          <motion.div
          ...
          >
            {/* Center Display with golden border */}
        ...
        {selectedNumber && !spinning && (
          ...
          >
            <p className="text-xl sm:text-2xl font-light">Your Lucky Number:</p>
